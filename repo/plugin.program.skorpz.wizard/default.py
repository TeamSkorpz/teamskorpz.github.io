...
def maintMenu(view=None):
	...
	if includevid == 'true':
		...
		addFile('--- Include Yoda: %s' % includeyoda.replace('true',on).replace('false',off), 'togglecache', 'includeyoda', icon=ICONMAINT, themeit=THEME3)
...
def thirdPartyInstall(name, url):
	...
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to preform a [COLOR %s]Fresh Install[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR green]Fresh Install[/COLOR][/B]")
	if type == 1:
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includejorel', 'includeyoda', 'includeseren', 'includeexodusredux', 'includekratos', 'includeclowns']
	titlelist = ['Include Video Addons', 'Include All Addons', 'Include Jorel', 'Include Yoda', 'Include Seren', 'Include Exodusredux', 'Include Kratos', 'Include clowns']
